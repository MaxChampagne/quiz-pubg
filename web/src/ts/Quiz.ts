...
export class Quiz {
    ...
    private objJSONQuiz;
    private messageResultat:string;
    private btnVerifierActiver:boolean = false;
    private btnValiderReponse:JQuery = null;
     ...
     */
    private afficherQuestion(no:number):any{
        console.log(no);
     ...
     */
    private cliquerMonChoix(evenement: Event): any {
        this.btnValiderReponse
    ...
    */
    private cliquerBtnValiderMonChoix(evenement: Event): any {

        let choixReponse = $('#Q' + this.questionActive)
            .find('li input:checked')
            .val();
        let reponse = Array('A', 'B', 'C', 'D');
        ...
        if ($('#Q'+this.questionActive).find('input:checked')) {
            ...
            for(let i=0; i <= reponse.length; i ++){
                let lesReponse = $('#Q'+this.questionActive)
                                        .find($('#Q'+this.questionActive+reponse[i]+ ' ~ label > div'));
                let lesReponseVal = $('#Q'+this.questionActive+reponse[i]).val();
    ...
    private afficherResultat(num:number){
        $('#pointage').hide();
        if(num == 1){
     ...
     * @param {event} evenement -> click
     */
    private cliquerBtnProchaineQuestion(): any {
        this.btnValiderReponse
            .prop("class", "");
